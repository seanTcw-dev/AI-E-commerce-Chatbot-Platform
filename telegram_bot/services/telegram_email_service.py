...
class TelegramEmailService:
    ...
    def send_agent_notification(self, session_id, host_url):
        ...
        try:
            ...
            template_paths = [
                os.path.join(templates_dir, 'telegram_notification.txt'),
                os.path.join(chatbot_dir, 'templates', 'telegram_notification.txt'),
                os.path.join('templates', 'telegram_notification.txt'),
                os.path.join('..', 'templates', 'telegram_notification.txt'),
                os.path.join('..', '..', 'templates', 'telegram_notification.txt')
            ]
            ...
            if text_template is None:
                raise FileNotFoundError("Could not find telegram_notification.txt template")
        ...
        try:
            # Create a dummy Flask app to provide context for render_template
            app = Flask(__name__, template_folder=templates_dir)
            with app.app_context():
                html_content = render_template('telegram_notification.html', 
                                             session_id=session_id, 
                                             timestamp=timestamp, 
                                             chat_link=chat_link)
        except Exception as e:
